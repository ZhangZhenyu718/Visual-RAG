...
CLIP [@radford2021clip] established that a dual-encoder trained with a
...
here [@schuhmann2022laion; @cherti2023openclip]. SigLIP [@zhai2023siglip]
...
[@lewis2020rag; @guu2020realm], classically over dense passage
embeddings [@karpukhin2020dpr]. Three RAG sub-problems recur in this
dissertation in video form.

**Query–document mismatch.** When queries and indexed content live in different
registers, rewriting the query helps: HyDE [@gao2023hyde] generates a
hypothetical document and searches with its embedding; least-to-most prompting
[@zhou2023leasttomost] decomposes complex questions into simpler sub-problems. The
...
"caption". Multiple rewrites are combined with reciprocal rank fusion [@cormack2009rrf], whose rank-based, score-free formulation is what makes fusing
...
[@nogueira2019bertrerank], with the BGE family [@chen2024bgem3] among the
...
**Self-critique.** Self-RAG [@asai2024selfrag] trains a model to critique its
...
Chain-of-thought prompting [@wei2022cot] established that intermediate
reasoning improves LLM task performance; ReAct [@yao2023react] interleaved
...
agents in this project. Toolformer [@schick2023toolformer] demonstrated that tool
...
the W4 agent is built directly. Reflexion [@shinn2023reflexion] added verbal
...
A rapidly growing line of work applies LLM orchestration to video. LLoVi [@zhang2024llovi] shows a deliberately simple recipe — densely caption short clips,
...
controller: Wang et al. [@wang2024videoagent] iteratively *search* for informative frames,
answering NExT-QA with ~8 frames on average, while Fan et al. [@fan2024videoagent] equip the
LLM with a structured memory over events and object tracks, queried through
tools. VideoRAG [@ren2025videorag] scales the RAG framing to extremely long
...
finding [@xiao2024nextgqa] shows is missing in practice — is an account of how
