...
**Dataset.** All experiments use the validation split of NExT-QA [@xiao2021nextqa], a benchmark of causal and temporal multiple-choice questions over short
(~44 s) everyday videos, together with the temporal grounding labels contributed
by NExT-GQA [@xiao2024nextgqa].
...
information a question asks about is often simply *absent* from the speech — a
question about what a child *does* is rarely answered by what anyone *says*. And
the CLIP text encoder that embeds both queries and transcripts is trained
predominantly on English alt-text, so even present information may be *unreadable*
to it. These two causes are entangled in this experiment; §6.2 explains why the
present design cannot apportion blame between them, and names the
...
score = α·visual + (1−α)·text. Figure 5.1 sweeps α over
...
(+9.9% relative), R@5 +6.7%, MRR +6.4% — while leaving the top of the ranking
essentially untouched (R@1 .026 → .025, tIoU@1 unchanged at .035; Table 5.2,
rows 1–2; Figure 5.2). The pattern is informative: translating
...
at 0.8 it remained below the no-reranking baseline. One asymmetry should be stated
plainly: this negative result rests on 100 questions, whereas every positive
retrieval result in this chapter is measured on all 3,358. The component was
rejected on the development subset and never promoted to a full-split run, so the
evidence against it is weaker than the evidence for the components that replaced
it — a decision revisited in §6.3. The mechanism mirrors §5.3.1:
...
and Figure 5.2 show the ladder at τ = 0.5, corpus scope:
...
intervals of arbitrary length. Table 5.4 repeats the ViT-B baseline and the best
...
**Table 5.4 — Threshold sensitivity (visual modality, ViT-B index).**
...
Table 5.5 and Figure 5.3 compare the two agent designs of
Chapter 3 on the first 150 grounded val questions under a text-only LLM
(deepseek-chat [@deepseek2024v3]): the single-loop function-calling agent (W4)
and the LangGraph
...
**Table 5.5 — Five-choice QA accuracy, 150 val questions.** Three variables move
across these rows — agent, base model, and evidence channel — so only rows sharing
two of the three are directly comparable; §5.4.3 draws the valid pairings. TP is
omitted from the by-type columns (n = 2) and reported in Appendix B.

| Agent | Base model | Evidence | Overall | CW (n=61) | TN (n=44) | TC (n=22) | CH (n=21) |
|---|---|---|---|---|---|---|---|
| Simple loop (W4) | deepseek-chat | text-only | .447 | .459 | .341 | .591 | .429 |
| LangGraph (W7) | deepseek-chat | text-only | .547 | .590 | .341 | .773 | .619 |
| none — prior only (§5.4.3) | deepseek-chat | none | .560 | .574 | .568 | .545 | .524 |
| none — prior only (§5.4.3) | claude-opus-4-8 | none | .560 | .639 | .455 | .591 | .476 |
| LangGraph (W7), best config (§5.4.4) | claude-opus-4-8 | multimodal | **.727** | **.852** | .568 | .682 | **.762** |
...
re-run under the *simple* agent with a single change from its own text-only row:
the provider was switched to a multimodal LLM (claude-opus-4-8), whose tool
results embed each segment's keyframe image alongside the transcript. Retrieval
index, tools, prompts, and questions were held fixed. The agent is named because
it matters for the comparison: the LangGraph harness targeted OpenAI-compatible
providers only at the time of this experiment, so the strict single-variable
contrast is row 1 against row 3 of Table 5.6. The graph-agent row is listed
because it lands on the identical .341, which makes the same conclusion hold
against it; the graph-plus-multimodal combination was implemented later and is
evaluated in §5.4.4.

**Table 5.6 — Same 44 TN questions, evidence channel varied.**

| Configuration | TN accuracy |
|---|---|
| Text-only LLM, simple agent | .341 |
| Text-only LLM, graph agent (temporal tool) | .341 |
| **Multimodal LLM, simple agent, keyframes in evidence** | **.636** |
...
almost verbatim from the keyframes alone; Figure 5.4 shows
...
options, nothing else (Table 5.5, prior-only rows; 150/150 completed each).
...
across Table 5.5's rows conflate base-model strength, evidence channel, and
...
configuration reaches **.727** overall (Table 5.5, bottom row; 95% CI
...
One result inside that figure runs against the trend and should be stated rather
than averaged away. On the same 44 TN items, under the same multimodal evidence
channel, the graph agent's .568 is *three questions below* the simple agent's
.636 (§5.4.2): adding the temporal tool and the reflection pass did not help
here, and marginally hurt. At n = 44 a three-question gap is well inside sampling
noise and no claim of a real regression is made. The direction is nonetheless
consistent with a structural gap in the current design — the reflection node
audits a *text* rendering of the gathered evidence, so on questions whose answer
is a silent visual action it interposes a text-mediated check on a judgement the
model made from keyframes, and has nothing to contribute. This makes the
keyframe-aware reflection pass the one component interaction the dissertation
leaves untested (§6.2), and the headline TN figure for the evidence-channel
...
|---|---|---|
| Random | — | .200 |
| SeViLA zero-shot [@yu2023sevila] | full val, fine-tuned localiser | ≈.636 |
| LLoVi [@zhang2024llovi] | full val, GPT-based | ≈.677 |
| VideoAgent [@wang2024videoagent] | full val, GPT-4 + iterative frame selection | .713 |
| **This work, best config** | grounded 150-q sample, ≤6 segments/search | **.727** [.655, .798] |

| Grounding | Protocol | metric |
|---|---|---|
| FrozenBiLM (NG+) [@xiao2024nextgqa] | NExT-GQA test, answer grounding | mIoU .096 |
| Temp[CLIP] (NG+) [@xiao2024nextgqa] | NExT-GQA test, answer grounding | mIoU .121 |
| SeViLA [@xiao2024nextgqa] | NExT-GQA test, answer grounding | mIoU .217 |
...
The causal analyses of §5.2 and §5.3.1 blame the text channel's failure on a
property of the *domain* rather than of the components: NExT-QA's home videos
are speech-sparse, 60% of segments carrying any speech and much of that
multilingual chatter. That reading makes a falsifiable prediction — in a
...
44-question multimodal run ≈US$3. The retrieval programme and the text-only QA
studies — every experiment of §5.2, §5.3 and §5.5, plus the agent comparison of
§5.4.1 — are therefore reproducible on one consumer laptop plus ≈US$5 of API
usage. The one expensive run is the 150-question multimodal best configuration of
§5.4.4, at ≈US$25 (≈US$0.17 per question, dominated by keyframe image tokens),
bringing the full chapter to ≈US$30 end to end.
