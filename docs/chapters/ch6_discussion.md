...
prior-only baseline (§5.4.3) measures the prior directly for the multimodal
model: .560 with no evidence at all, which is why this dissertation rests its
QA claims on paired within-model comparisons rather than absolute accuracies.
The corresponding prior-only run for the text-only model would complete the
decomposition of the .341 floor specifically.
